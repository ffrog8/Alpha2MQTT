...
if [[ "${RS485_STUB:-0}" == "1" ]]; then
	BUILD_FLAGS="${BUILD_FLAGS} -DRS485_STUB=1"
	if [[ -n "${RS485_STUB_MODE:-}" ]]; then
		BUILD_FLAGS="${BUILD_FLAGS} -DRS485_STUB_MODE=${RS485_STUB_MODE}"
	fi
	if [[ -n "${RS485_STUB_FAIL_FIRST_N:-}" ]]; then
		BUILD_FLAGS="${BUILD_FLAGS} -DRS485_STUB_FAIL_FIRST_N=${RS485_STUB_FAIL_FIRST_N}"
	fi
	if [[ -n "${RS485_STUB_FAIL_REGISTER:-}" ]]; then
		BUILD_FLAGS="${BUILD_FLAGS} -DRS485_STUB_FAIL_REGISTER=${RS485_STUB_FAIL_REGISTER}"
	fi
fi
